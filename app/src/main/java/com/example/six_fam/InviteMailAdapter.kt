...
import com.example.six_fam.databinding.ItemInviteMailBinding

class InviteMailAdapter(
    private val listInvites: MutableList<String>,
    ...
    fun updateDataset(newList: List<String>) {
        listInvites.clear()
        listInvites.addAll(newList)
        notifyDataSetChanged()
    }
    fun removeInvite(mail: String) {
        listInvites.remove(mail)
        notifyDataSetChanged()
    }
}
